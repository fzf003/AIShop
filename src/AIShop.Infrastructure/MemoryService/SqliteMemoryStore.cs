...
// S2077（SQL 拼接）：本文件 SQL 的**值全部参数化**（AddWithValue），CommandText 拼接的仅为内部受控的
// 列名/表名/where 片段（BuildFilter 生成 / 常量），无用户输入直拼进 SQL，故禁用该规则。
...
public sealed class SqliteMemoryStore : IMemoryStore, IAsyncDisposable
{
    ...
    {
        Guard.NotNull(records);
        foreach (var record in records.Where(r => r.Embedding is not null))
        {
    ...
    private static void ValidateBatch(IReadOnlyList<MemoryVectorRecord> records)
    {
        ...
        foreach (var embedding in records.Select(r => r.Embedding))
        {
            ValidateEmbedding(embedding);
            if (embedding.Count != dimensions) throw new ArgumentException("SQLite vector records must have consistent embedding dimensions.", nameof(records));
        }
    }
