...
/// <summary>
/// DebugTelemetry 扩展：在 OTLP 导出前把 span 上 Debug 抓取的 HTTP 请求/响应 body 与 headers tag
/// 值替换为占位符，保证真实 body（含 Authorization / API key）不出进程。
///
/// 安全约束：Debug=true 时 EnrichWith 回调把含敏感信息的请求/响应 body（及 headers）写入
/// <c>System.Net.Http.HttpRequestOut</c> span。同一 span 会流经所有 processor，若不脱敏，
/// 这些 tag 会随 OTLP 导出到 Aspire Dashboard——违反「body 只写本地文件不进 OTLP」。
///
...
/// OTLP exporter <b>之前</b>（见 ServiceDefaults 接线）：FileSpanExporter 先同步落盘真实 body，
/// 本 processor 再覆盖值，OTLP 拿到的只有占位符。
/// 仅覆盖 Debug 抓取的 6 个 tag，保留 HttpClientInstrumentation 产出的标准 <c>http.*</c> 属性
...
public sealed class BodyRedactionProcessor : BaseProcessor<Activity>
{
    ...
    /// <summary>Debug 抓取的敏感 tag：请求/响应 headers（含 Authorization / API key）与 body。</summary>
